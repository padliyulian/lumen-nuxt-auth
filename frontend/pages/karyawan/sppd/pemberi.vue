<template>
    <div class="content-wrapper">
        <Header title="Menunggu Persetujuan Pemberi Tugas"/>
                                                ...
                                                <tbody>
                                                    <tr v-for="sppd in data.data" :key="sppd.id">
                                                        ...
                                                        <td>
                                                            ...
                                                            <a v-if="isPemberi(sppd.karyawan).length" @click.prevent="approveSppd(sppd.id)" href="#" title="Approve">
...
<script>
    ...
    export default {
        ...
        created() {
            this.getSppd(`${process.env.apiUrl}/karyawan/sppd/pemberi?page=1&length=${this.length}&column=${this.column}&dir=${this.dir}&search=${this.search}`)
        },

        mounted() { console.log(this.$auth.user.karyawan.id) },

        methods: {
            isPemberi(karyawans) {
                let pemberi = karyawans.filter(karyawan => {
            ...
            approveSppd(id) {
                this.form.post(`${process.env.apiUrl}/karyawan/sppd/pemberi/${id}`, {
                    headers: {
            ...
            lastPage() {
                this.getSppd(`${process.env.apiUrl}/karyawan/sppd/pemberi?page=${this.data.last_page}&length=${this.length}&column=${this.column}&dir=${this.dir}&search=${this.search}`)
            },
